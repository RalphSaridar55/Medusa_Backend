...
export default class AddProduct extends Component {
  ...
  submit = () => {
    ...
    if(this.state.range!==null){
      ...
      start = (new Date(this.state.range.firstDate).getTime())/1000;
      end = (new Date(this.state.range.secondDate).getTime())/1000;
    }
    if (this.state.variantImage < 1) {
      ...
      APIProduct.createProduct(sendingData).then((res)=>{
        console.log("RES: ",res);
        this.setState({loading:false})
        Alert.alert("Successful","Product has been created successfully");
      })
    }
  };
